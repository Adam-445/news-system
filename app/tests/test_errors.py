...
def test_not_found_error(client, regular_headers):
    ...
    invalid_id = uuid4()
    response = client.get(
        "/api/v1/users/search", params={"id": invalid_id}, headers=regular_headers
    )
    assert response.status_code == 404
...
def test_validation_error(client, regular_headers):
    ...
    response = client.get(
        "/api/v1/users/search", params={"id": "not-a-uuid"}, headers=regular_headers
    )
    assert response.status_code == 422
    print(f"Response: {response.json()}")
    assert response.json()["detail"][0]["field"] == "query.id"
