...
def test_regular_user_cannot_create_role(client, regular_headers):
    response = client.post(
        "/api/v1/admin/roles/invalid_role/permissions",
        json={"permission_name": "create_article"},
        headers=regular_headers,
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN


def test_moderator_cannot_delete_user(client, moderator_headers):
    response = client.delete(f"/api/v1/users/{UUID(int=0)}", headers=moderator_headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN
...
def test_role_hierarchy(client, admin_headers, moderator_headers, regular_headers):
    ...
    response = client.delete(f"/api/v1/articles/1", headers=regular_headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN
